// #! 🪧 IMAGERIE ECG LIST RISKGARD JSX
// #! 🔣 FR
...
//!! JSX
import { ImagerieECGFormRiskgard } from './RiskgardImagerieECGForm.jsx';
...
export function ImagerieECGListRiskgard() {
    // todo>> console
    ...
    const httpSuivieEcgPatients = 'http://127.0.0.1:3001/riskgard-patients-suivie-ecg';

    // id suivie ecg patient
    const [idSuivieEcgPatient, setIdSuivieEcgPatient] = useState(0);

    // tableau suivie ecg patients
    const [
        tableauSuivieEcgPatient,
        setTableauSuivieEcgPatient
    ] = useState([]);
    ...
    useEffect(() => {
        ...
        // récupération des données des suivie ecg patients
        fetch(httpSuivieEcgPatients)
            .then(response => response.json())
            .then(data => {
                setTableauSuivieEcgPatient(data);
                setIdSuivieEcgPatient(data.length + 1);
            })
